...
const DoctorAppointmentsPage = () => {
  ...
  const fetchAppointments = async (email) => {
    try {
      const response = await fetch(`/doctor/appointments/${email}`, {
        method: 'GET',
        headers: {
    ...
    <div>
      ...
      <div className="appointments-container">
        ...
        {appointments.map((appointment) => (
          <AppointmentTile key={appointment.APPOINTMENT_ID} appointment={appointment} userEmail={appointment.PATIENT_EMAIL} />
        ))}
      </div>
    </div >
  );
